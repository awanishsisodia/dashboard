...
def aboutus_view(request):
    return render(request,'db/aboutus.html')
...
#            send_mail(str(name)+' || '+str(email),message,contact_no, settings.EMAIL_HOST_USER, settings.EMAIL_RECEIVING_USER, fail_silently = False)
            return render(request, 'db/contactussuccess.html')
    return render(request, 'db/contactus.html', {'form':sub})
@login_required(login_url='customerlogin')
def cameradashveiw(request):
    cameradash=models.CameraDashboard.objects.all()
